...
select 区域, 
	...
	sum([May Spending Forecast]) as May_20,
	sum([Jun Spending Forecast]) as Jun_20,
    sum([2020Apr]+[2020May]+[2020Jun]) as Q2_Total_Spending_QTD
from P4P_20191002
where 端口 not like '%wrong%' and 区域 <> '-'
group by 区域
order by 区域
...
sql = '''
select a.AM, 
	sum([Q2 Total Spending Forecast]) as Q2_Total_Spending_Forecast, 
	sum([Apr_FX GAIN(RMB)]) as Apr_FX_Gain_RMB, 
	sum([May_FX GAIN(RMB)]) as May_FX_Gain_RMB, 
	sum([Jun_FX GAIN(RMB)]) as Jun_FX_Gain_RMB,
...
select a.NB, 
	a.销售, 
	sum([Q2 Eligible Spending Forecast]) as Q2_Eligible_Spending_Forecast, 
	sum([FX GAIN(RMB)_Sales]) as FX_Gain_RMB_Sales,
	sum([FX GAIN(RMB)_Sales])/0.18 as FX_Gain_Spending_RMB_Sales, 
	sum([Eligible Spending(QTD)]) as Q2_Eligible_Spending_QTD, 
...
select a.NB
	, a.销售
	, sum([Apr Eligible Spending]) as Apr_Eligible_Spending
	, sum([May Eligible Spending]) as May_Eligible_Spending
	, sum([Jun Eligible Spending]) as Jun_Eligible_Spending
	, sum([Apr Eligible Spending Forecast]) as Apr_Eligible_Spending_Forecast
	, sum([May Eligible Spending Forecast]) as May_Eligible_Spending_Forecast
	, sum([Jun Eligible Spending Forecast]) as Jun_Eligible_Spending_Forecast
    , sum([Q2 Eligible Spending Forecast]) as Q2_Eligible_Spending_Forecast
    , sum([Eligible Spending(QTD)]) as Q2_Eligible_Spending_QTD
from P4P_20191002 a 
...
select a.NB
	, a.销售
	, sum([Apr Eligible Spending]+[Apr Eligible Spending]*0.0789313904068002/0.18) as Apr_Eligible_Spending
...
select b.AM_Region
	...
	, sum([Apr Spending Forecast]) as Apr_20
	, sum([May Spending Forecast]) as May_20
	, sum([Jun Spending Forecast]) as Jun_10
...
select b.AM_Region
	, b.AM_Group
	,b.AM
	, sum([Apr Spending Forecast])+sum([Apr Spending Forecast])*0.0789313904068002/0.18 as Apr_20
	, sum([May Spending Forecast])+sum([May Spending Forecast])*0.0789313904068002/0.18 as May_20
	, sum([Jun Spending Forecast])+sum([Jun Spending Forecast])*0.0789313904068002/0.18 as Jun_20
