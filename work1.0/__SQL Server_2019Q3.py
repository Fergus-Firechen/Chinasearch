...
# P4P
sql = '''
select 区域, sum([Jul Spending Forecast]) as Jul_19,
sum([Aug Spending Forecast]) as Aug_19,
sum([Sep Spending Forecast]) as Sep_19,
sum([2019Jul]+[2019Aug]+[2019Sep]) as Q3_Total_Spending_QTD
from P4P_20190402
where 端口 not like '%wrong%' and 区域 <> '-'
group by 区域
order by 区域
...
# P4P
sql = '''
select 财务做账区域, 区域, sum([Jul Spending Forecast]) as Jul_19,
sum([Aug Spending Forecast]) as Aug_19,
sum([Sep Spending Forecast]) as Sep_19,
sum([2019Jul]+[2019Aug]+[2019Sep]) as Q3_Total_Spending_QTD
from P4P_20190402
...
# P4P
sql = '''
select a.AM, sum([Q3 Total Spending Forecast]) as Q3_Total_Spending_Forecast, 
sum([Jul_FX GAIN(RMB)]) as Jul_FX_Gain_RMB, sum([Aug_FX GAIN(RMB)]) as Aug_FX_Gain_RMB, sum([Sep_FX GAIN(RMB)]) as Sep_FX_Gain_RMB,
...
sht = wb.sheets['Sales']
sql = '''
select a.NB, a.销售, sum([Q3 Eligible Spending Forecast]) as Q3_Eligible_Spending_Forecast, sum([FX GAIN(RMB)_Sales]) as FX_Gain_RMB_Sales,
sum([FX GAIN(RMB)_Sales])/0.18 as FX_Gain_Spending_RMB_Sales, sum([Eligible Spending(QTD)]) as Q3_Eligible_Spending_QTD, 
...
rows100 = sht['A1'].current_region.rows.count
sql = '''
select a.销售, [NB Month], sum([Q3 Eligible Spending Forecast]) as Q3_Eligible_Spending_Forecast, sum([Q3 Eligible GP Forecast]) as Q3_Eligible_GP_Forecast
...
sql = '''
select a.销售, sum([Q3 Eligible Spending Forecast]) as Q3_Eligible_Spending_Forecast, 
sum([Q3 Eligible GP Forecast]) as Q3_Eligible_GP_Forecast
from P4P_20190402 a inner join [CSA_Sales] b on a.销售=b.Sales
where 端口 not like '%wrong%' and a.NB='2019Q3'
group by a.销售
order by a.销售
    '''
# P4P
...
sht = wb.sheets[0]
sql = '''
select a.NB, a.销售, sum([Jul Eligible Spending]) as Jul_Eligible_Spending, sum([Aug Eligible Spending]) as Aug_Eligible_Spending, 
sum([Sep Eligible Spending]) as Sep_Eligible_Spending, sum([Jul Eligible Spending Forecast]) as Jul_Eligible_Spending_Forecast, 
sum([Aug Eligible Spending Forecast]) as Aug_Eligible_Spending_Forecast, sum([Sep Eligible Spending Forecast]) as Sep_Eligible_Spending_Forecast, 
sum([Q3 Eligible Spending Forecast]) as Q3_Eligible_Spending_Forecast, sum([Eligible Spending(QTD)]) as Q3_Eligible_Spending_QTD
from P4P_20190402 a inner join [CSA_HK_Sales] b on a.销售=b.Sales
...
# sql
sql = '''
select a.NB, a.销售, sum([Jul Eligible Spending]+[Jul Eligible Spending]*0.0789313904068002/0.18) as Jul_Eligible_Spending, 
sum([Aug Eligible Spending]+[Aug Eligible Spending]*0.0789313904068002/0.18) as Aug_Eligible_Spending, 
sum([Sep Eligible Spending]+[Sep Eligible Spending]*0.0789313904068002/0.18) as Sep_Eligible_Spending, 
sum([Jul Eligible Spending Forecast]+[Jul Eligible Spending Forecast]*0.0789313904068002/0.18) as Jul_Eligible_Spending_Forecast, 
sum([Aug Eligible Spending Forecast]+[Aug Eligible Spending Forecast]*0.0789313904068002/0.18) as Aug_Eligible_Spending_Forecast, 
sum([Sep Eligible Spending Forecast]+[Sep Eligible Spending Forecast]*0.0789313904068002/0.18) as Sep_Eligible_Spending_Forecast, 
sum([Q3 Eligible Spending Forecast]+[Q3 Eligible Spending Forecast]*0.0789313904068002/0.18) as Q3_Eligible_Spending_Forecast, 
sum([Eligible Spending(QTD)]+[Eligible Spending(QTD)]*0.0789313904068002/0.18) as Q3_Eligible_Spending_QTD
...
sht = wb.sheets[0]
sql = '''
select b.AM_Region, b.AM_Group,b.AM, sum([Jul Spending Forecast]) as Jul_19,
sum([Aug Spending Forecast]) as Aug_19,
sum([Sep Spending Forecast]) as Sep_19,
sum([Total Spending]) as Q3_Total_Spending_QTD
from P4P_20190402 a inner join [CSA_AM] b on a.AM=b.AM
where 端口 not like '%wrong%'
...
sql = '''
select b.AM_Region, b.AM_Group,b.AM, sum([Jul Spending Forecast])+sum([Jul Spending Forecast])*0.0789313904068002/0.18 as Jul_19,
sum([Aug Spending Forecast])+sum([Aug Spending Forecast])*0.0789313904068002/0.18 as Aug_19,
sum([Sep Spending Forecast])+sum([Sep Spending Forecast])*0.0789313904068002/0.18 as Sep_19,
sum([Total Spending])+sum([Total Spending])*0.0789313904068002/0.18 as Q3_Total_Spending_QTD
